...
class TriMipRFModel(RFModel):
    ...
    def rendering(
        ...
        features, sigmas = rgb_sigma_fn(t_starts, t_ends, ray_indices.long())
        ...
        sample_buffer = {
            'num_samples': torch.as_tensor(
                [len(t_starts)], dtype=torch.int32, device=features.device
            ),
        }

        # Rendering: accumulate rgbs, opacities, and depths along the rays.
        feature = accumulate_along_rays(
            weights, ray_indices=ray_indices, values=features, n_rays=n_rays
        )

        ray_dirs = (rays.directions + 1.0) / 2.0

        d = self.field.direction_encoding(ray_dirs.view(-1, ray_dirs.shape[-1]))

        h = torch.cat([d, feature.view(-1, self.field.geo_feat_dim)], dim=-1)

        colors = self.field.mlp_head(h).view(list(feature.shape[:-1]) + [3])
